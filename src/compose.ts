...
export function generateComposeCode(style: Record<string, string>): string {
  ...
  if (style.margin) {
    ...
    const args = []
    if (m['android:layout_marginStart']) args.push(`start = ${toComposeUnit(m['android:layout_marginStart'])}`)
    if (m['android:layout_marginTop']) args.push(`top = ${toComposeUnit(m['android:layout_marginTop'])}`)
    if (m['android:layout_marginEnd']) args.push(`end = ${toComposeUnit(m['android:layout_marginEnd'])}`)
    if (m['android:layout_marginBottom']) args.push(`bottom = ${toComposeUnit(m['android:layout_marginBottom'])}`)
    if (m['android:layout_margin']) args.push(`${toComposeUnit(m['android:layout_margin'])}`) 
  ...
  if (style.padding) {
    ...
    const args = []
    if (p['android:paddingStart']) args.push(`start = ${toComposeUnit(p['android:paddingStart'])}`)
    if (p['android:paddingTop']) args.push(`top = ${toComposeUnit(p['android:paddingTop'])}`)
    if (p['android:paddingEnd']) args.push(`end = ${toComposeUnit(p['android:paddingEnd'])}`)
    if (p['android:paddingBottom']) args.push(`bottom = ${toComposeUnit(p['android:paddingBottom'])}`)
    if (p['android:padding']) args.push(`${toComposeUnit(p['android:padding'])}`)
  ...
  if (composableName === 'Text') {
    ...
    if (style['font-size']) params.push(`fontSize = ${convertComposeUnit(style['font-size'], 'sp')}`)
    if (style['line-height']) params.push(`lineHeight = ${convertComposeUnit(style['line-height'], 'sp')}`)
    if (style['letter-spacing']) params.push(`letterSpacing = ${convertComposeUnit(style['letter-spacing'], 'sp')}`)
    
    if (style['font-weight']) {
      const fw = style['font-weight']
      if (fw === 'bold' || parseInt(fw) >= 700) {
        params.push(`fontWeight = FontWeight.Bold`)
      } else if (parseInt(fw) < 400) {
        params.push(`fontWeight = FontWeight.Light`)
      } else if (parseInt(fw) === 500) {
    ...
    if (style['text-align']) {
      ...
      else if (align === 'justify') params.push(`textAlign = TextAlign.Justify`)
    }
    ...
    if (composableName === 'Column') {
      ...
      else if (style['justify-content'] === 'flex-end') params.push(`verticalArrangement = Arrangement.Bottom`)
      else if (style['justify-content'] === 'space-between') params.push(`verticalArrangement = Arrangement.SpaceBetween`)
      
      if (style['align-items'] === 'center') params.push(`horizontalAlignment = Alignment.CenterHorizontally`)
      else if (style['align-items'] === 'flex-end') params.push(`horizontalAlignment = Alignment.End`)
    } else {
      if (style['justify-content'] === 'center') params.push(`horizontalArrangement = Arrangement.Center`)
      else if (style['justify-content'] === 'flex-end') params.push(`horizontalArrangement = Arrangement.End`)
      else if (style['justify-content'] === 'space-between') params.push(`horizontalArrangement = Arrangement.SpaceBetween`)
      
      if (style['align-items'] === 'center') params.push(`verticalAlignment = Alignment.CenterVertically`)
      else if (style['align-items'] === 'flex-end') params.push(`verticalAlignment = Alignment.Bottom`)
    }
  }
