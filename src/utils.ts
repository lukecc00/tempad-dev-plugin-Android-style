...
function detectTagName(style: Record<string, string>): string {
    // 1. ScrollView
    if (style['overflow-y'] === 'scroll' || style['overflow-y'] === 'auto') return 'ScrollView';
    if (style['overflow-x'] === 'scroll' || style['overflow-x'] === 'auto') return 'HorizontalScrollView';

    // 2. CardView (Shadow + Radius)
    if (style['box-shadow']) return 'androidx.cardview.widget.CardView';

    // 3. ImageView
    if ((style['background-image'] && style['background-image'].includes('url')) || style['object-fit']) return 'ImageView';

    // 4. LinearLayout (Flex)
    if (style['display'] === 'flex') return 'LinearLayout';

    // 5. TextView (Text content)
    // Strong signals for text
    if (style['font-family'] || style['color'] || style['font-size'] || style['text-align'] || style['line-height'] || style['text-overflow']) return 'TextView';
    ...
    if (hasDim && hasBg && !isContainer) return 'View';
...
export function cssToAndroidAttrs(style: Record<string, string>, tagName: string): AndroidAttributes {
  ...
  // Max/Min Size
  if (style['max-width']) attrs['android:maxWidth'] = convertUnit(style['max-width'], 'dp')
  if (style['min-width']) attrs['android:minWidth'] = convertUnit(style['min-width'], 'dp')
  if (style['max-height']) attrs['android:maxHeight'] = convertUnit(style['max-height'], 'dp')
  if (style['min-height']) attrs['android:minHeight'] = convertUnit(style['min-height'], 'dp')
  ...
  if (style['background-color']) {
    attrs[bgProp] = style['background-color']
     ...
     const match = style.background.match(/var\(--([a-zA-Z0-9_-]+)\)/)
     if (match) {
  ...
  if (tagName === 'TextView') {
      if (style.color) attrs['android:textColor'] = style.color
      if (style['font-size']) attrs['android:textSize'] = convertUnit(style['font-size'], 'sp')
      ...
      if (style['text-align']) {
        ...
        let gravity = ''
        if (align === 'center') gravity = 'center_horizontal'
        else if (align === 'right') gravity = 'end'
        else if (align === 'left') gravity = 'start'
  ...
  if (tagName === 'ImageView') {
      ...
      if (style['object-fit']) {
          if (style['object-fit'] === 'cover') attrs['android:scaleType'] = 'centerCrop'
          else if (style['object-fit'] === 'contain') attrs['android:scaleType'] = 'centerInside'
      }
  }
  ...
  if (tagName !== 'TextView') {
       if (style['justify-content'] === 'center' || style['align-items'] === 'center') {
          ...
          if (tagName === 'LinearLayout') {
              ...
              if (style['justify-content'] === 'center') gravity += 'center_horizontal'
              if (style['align-items'] === 'center') gravity += (gravity ? '|' : '') + 'center_vertical'
          }
...
export function generateAndroidTag(style: Record<string, string>): string {
    ...
    if (tagName === 'TextView') {
        if (!attrs['android:text']) attrs['android:text'] = '@string/some_text'
    ...
    const sortedKeys = Object.keys(attrs).sort((a, b) => {
        const priority = (key: string) => {
            if (key.startsWith('xmlns:')) return 0
